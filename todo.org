- the `cicada/simple.ts` game play might be enough for dependent game (dependent type)
- should arrow_t also use `() => game_t` ? yes.
- use `module_t` for better API
- syntax should be plugin of the semantics ?
