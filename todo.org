- change of bases
- test image and kernel by "four ways to glue a square"
- homology group of cell-complex
  - image (boundary (n))
    is subgroup of
    kernel (boundary (n+1))
  - homology group =
    quotient (kernel (boundary (n+1)), image (boundary (n)))
  - use bases of
    kernel (boundary (n+1))
    to represent bases of
    image  (boundary (n))
- abelian_group_t
...
* [bug] limitation
- if the number goes beyond `number`, we will need generic `matrix_t <F>`
- generic matrix_t and vector_t
- number.ts
- generic matrix_t algos
- test with decimal.js -- test K1
- understand int_solve -- own study
- understand quotient module -- own study
* [note] row v.s. col
- `row_trans`
