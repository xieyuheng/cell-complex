...
- `this`
  ...
  - how to solve unification problem with side-effect ?
    ...
    - when a `this` occurs, make a ref to the field,
      and update the field by current partial game
  - API of `this_t` in `record_t` and `union_t` map
    1. just use `this_t`
       and let `game_cons_t` travel through freshly constructed game
       and replace `this` with `this_ref`
    2. use some callback API --- how ???
       a callback whose arg is the root of `this_ref`
- `record_cons_t` and `union_cons_t` as `game_cons_t`
- `module.game ()` call `game_cons.start ()`
- "prelude.ts" in `std/`
- `module.use ()`
